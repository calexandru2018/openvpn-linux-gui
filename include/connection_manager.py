...
class ConnectionManager():
	def __init__(self, rootDir, user_man_folder_name, server_man_folder_name):
		...
		self.new_ip = False
		self.project_name = "openvpn-linux-gui"
	...
	def generate_ovpn_file(self):
		...
		try:
			...
			path = self.rootDir+"/"+self.server_man_folder_name+"/"+country.upper() + "."+ self.fileType
			...
			if self.file_manager.returnFileExist(self.user_man_folder_name, self.ovpn_file[0], self.ovpn_file[1]):
				self.file_manager.deleteFile(self.user_man_folder_name, self.ovpn_file[0], self.ovpn_file[1])
			if self.file_manager.createFile(self.user_man_folder_name, self.ovpn_file[0], self.ovpn_file[1], serverReq.text):
	...
	def generate_ovpn_for_boot(self):
		...
		modified_request = original_req[:start_index+14] + " /opt/openvpn-gui-linux/.user_credentials" + original_req[start_index+14:]
	...
	def copy_credentials(self):
		cmds = ["mkdir /opt/"+self.project_name+"/", "cp " + self.rootDir + "/" + self.user_man_folder_name + "/.user_credentials /opt/"+self.project_name+"/"]
		try:
			if(not os.path.isdir("/opt/"+self.project_name+"/")):
				for cmd in cmds:
					subprocess.run(["sudo", "bash", "-c", cmd])
			else:
